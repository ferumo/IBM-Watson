...
spacex_df = pd.read_csv("09_Capstone_proj\spacex_launch_dash.csv")
...
def get_scatter_plot(entered_site, slider_range):
    filtered_df = spacex_df[spacex_df['Launch Site']==str(entered_site)]
    if entered_site == 'ALL':
        range_df = spacex_df[(spacex_df['Payload Mass (kg)']>=slider_range[0])&(spacex_df['Payload Mass (kg)']<=slider_range[1])]
        fig = px.scatter(range_df, x='Payload Mass (kg)', y='class', color='Booster Version Category', 
        title='Correlation between Payload and Success for all Sites')
        return fig
    else:
        # return the filtered scatter plot
        range_df = filtered_df[(filtered_df['Payload Mass (kg)']>=slider_range[0])&(filtered_df['Payload Mass (kg)']<=slider_range[1])]
        fig = px.scatter(range_df, x='Payload Mass (kg)', y='class', color='Booster Version Category', 
        title='Correlation between Payload and Success for all Sites')
        return fig
